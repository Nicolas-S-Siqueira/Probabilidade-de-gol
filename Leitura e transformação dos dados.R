...
#Competições
#Importar um json como um dataframe
...
#Ler as partidas
#### pra não precisar carregar 1 arquivo de cada vez
...
#adicionar a informação dos passes q geraram as finalizações
...
#função pra calcular a distancio do chute dada a coordenada
...
#adicionando distancia e angulo do chute
xg_data <- xg_data %>%
...
#função pra calcular a distancio do defensor para o local do chute
# dada a coordenada
...
#angulos bloqueadores
#usar essa função pra calcular a variancia entre os angulos e com isso ver
...
#tirar os NULLs na lista do shot.freeze_frame (toda a linha)
xg_data <- xg_data %>%  
...
#função q recebe um elemento do shot.freeze.frame e faz o espelhamento
...
# a func. recebe o dataframe q é um elemento da lista. tem q ser usada em conjunto com o map se quiser percorrer a lista toda
...
gk_goal <- function(x,y,z) { 
  ...
  shot.Y <- y
  post1.X = 120
  post1.Y = 44 + 1 # adicionando um pouco por causa da curva da bola e por causa do tempo q o goleiro tem pra se mover durante a trajetoria do chute
  post2.X = 120
  post2.Y = 36 - 1 # adicionando um pouco por causa da curva da bola e por causa do tempo q o goleiro tem pra se mover durante a trajetoria do chute
...
qtd_def_2m <- function(x,y,z) { 
  ...
  #retira as distancias que são 10m ou maiores e soma o inverso das outras distâncias
...
qtd_def_in_way <- function(x,y,z) { 
  ...
  valores <- map2(def.X, def.Y, function(def.X, def.Y){
    ...
    shot.Y <- y
    post1.X = 120
    post1.Y = 44 + 1 # adicionando um pouco por causa da curva da bola e por causa do tempo q os defensores têm pra se mover durante a trajetória do chute
    post2.X = 120
    post2.Y = 36 - 1 # adicionando um pouco por causa da curva da bola e por causa do tempo q os defensores têm pra se mover durante a trajetória do chute
  ...
  return(sum(valores)) #soma de um vetor lógico retorna a qauntidade de TRUE's
}
...
ang_blocks <- function(x,y,z) { 
  ...
  # posicao x do defensor  
  def.X <-   z %>%
    dplyr::filter(teammate == FALSE & position.id != 1) %>%
    dplyr::pull(location) %>%
    purrr::map(1) 
  
  # posicao y do defensor
  def.Y <-   z %>%
    dplyr::filter(teammate == FALSE & position.id != 1) %>%
    dplyr::pull(location) %>%
    purrr::map(2)
  ...
  return(ifelse(test = is.na(angs), yes = 0, no = angs)) #soma de um vetor lógico retorna a qtd de TRUEs
}
...
#Função que calcula o Ângulo do passe em relação ao jogador
# que recebe e o centro do gol.
